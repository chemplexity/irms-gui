...
% File Menu --> Load
% ---------------------------------------
obj.menu.loadThermo   = newMenu(obj.menu.file.load, 'Thermo data (*.CF)');
...
obj.menu.loadThermo.Callback   = {@loadThermoCallback, obj};
...
% Load Thermo (*.CF)
% ---------------------------------------
function loadThermoCallback(~, ~, obj)
...
statusText = 'Saving sample information and peak data to CSV file... ';
...
if ischar(fileName) && ischar(filePath)
    ...
    for i = 1:size(y,1)
        ...
        for j = 1:size(y,2)
            ...
            switch x
                case 'char'
                    y{i,j} = regexprep(y{i,j}, '([,]|\t)', ' ');
                    y{i,j} = deblank(strtrim(y{i,j}));
